...
export type BrainGitHubEnv = Record<string, unknown> & {
  GITHUB_PAT?: string;
  GITHUB_TOKEN?: string;
...
export async function getBrain(env: AnyEnv, options: GetBrainOptions = {}): Promise<string> {
  const repo = determineRepo(env, options.repo);
  const preferredBranch = determineBranchPreference(env, options.branch ?? null);
  const path = determinePath(env, options.path);
  const branches = uniqueBranches(preferredBranch, options.branches);

  for (const branch of branches) {
    const payload = await fetchBrainFromGitHub(env, repo, branch, path);
    if (typeof payload === 'string' && payload.length > 0) {
      console.log('[getBrain] Loaded brain.md from GitHub', { repo, branch, path, bytes: payload.length });
      return payload;
    }
  }
