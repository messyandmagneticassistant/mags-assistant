...
import { getBrain, type BrainGitHubEnv } from './getBrain';

type BrainKvNamespace = {
  get: (
    key: string,
...
export type BrainSyncEnv = BrainGitHubEnv & {
  BRAIN?: BrainKvNamespace | null;
  POSTQ_KV_ID?: string;
...
export async function putBrainToKV(env: BrainSyncEnv): Promise<PutBrainResult> {
  ...
  try {
    ...
    if (kv && typeof kv.get === 'function') {
      const raw = await kv.get(KV_BRAIN_KEY, 'text');
      if (raw) {
