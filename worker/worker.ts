  ...
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === '/brain') {
        ...
        try {
          const markdown = await getBrain(env);
          if (!markdown || !markdown.trim()) {
            return jsonResponse({ ok: false, error: 'brain-not-found' }, { status: 404 });
          }
          ...
          console.error('[worker:/brain] failed to load brain.md', err);
          return jsonResponse({ ok: false, error: 'brain-fetch-failed' }, { status: 500 });
        }
      }
